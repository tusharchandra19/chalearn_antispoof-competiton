...
def list_image(root, recursive, exts):
    ...
    Returns
    -------
    File that contains all the image under the specified path
    """
        ...
        for path, dirs, files in os.walk(root, followlinks=True):
            ...
            for fname in files:
                ...
                if os.path.isfile(fpath) and (suffix in exts):
                    ...
                    i += 1
        for j, k in sorted(cat.items(), key=lambda x: x[1]):
            relpath = os.path.relpath(j, root)
            ...
            if fname.startswith(args.prefix) and fname.endswith('.lst'):
                ...
                if args.num_thread > 1 and multiprocessing is not None:
                    ...
                    for i, item in enumerate(image_list):
                        ...
                        if counting % 1000 == 0:
                            cur_time = time.time()
                            print('time:', cur_time - pre_time, ' count:', counting)
                            pre_time = cur_time
                        counting += 1
        if not counter:
